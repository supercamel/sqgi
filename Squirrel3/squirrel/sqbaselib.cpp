...
struct AsyncPendingInvocation {
    HSQOBJECT generator;
    HSQOBJECT callback;
    SQBool has_callback;
};

typedef std::deque<AsyncPendingInvocation> AsyncPendingQueue;
typedef std::map<SQClosure *, AsyncPendingQueue> AsyncPendingMap;
static AsyncPendingMap g_async_pending;
...
static SQInteger closure_call(HSQUIRRELVM v)
{
    ...
    if (sq_type(self) == OT_CLOSURE && (_closure(self)->_function->_bgenerator == false))
	{
		return sq_tailcall(v, sq_gettop(v) - 1);
	}
    if(SQ_FAILED(sq_call(v, sq_gettop(v) - 1, SQTrue, SQTrue))) {
        return SQ_ERROR;
    }

    /* For generator-style async closures, queue direct calls so `.end()` can
     * resume the pending invocation (matching begin/end async flow). */
    if(sq_type(self) == OT_CLOSURE && _closure(self)->_function->_bgenerator) {
        if(sq_gettype(v, -1) == OT_GENERATOR) {
            AsyncPendingInvocation pending;
            sq_resetobject(&pending.generator);
            sq_resetobject(&pending.callback);
            pending.has_callback = SQFalse;

            sq_getstackobj(v, -1, &pending.generator);
            sq_addref(v, &pending.generator);
            g_async_pending[_closure(self)].push_back(pending);
        }
    }
...
static void async_release_pending(HSQUIRRELVM v, AsyncPendingInvocation &p)
{
    sq_release(v, &p.generator);
    if(p.has_callback) {
        sq_release(v, &p.callback);
    }
}

static SQInteger closure_begin(HSQUIRRELVM v)
{
    SQObject &self = stack_get(v, 1);
    if(sq_type(self) != OT_CLOSURE) {
        return sq_throwerror(v, _SC("begin can be called only on closures"));
    }

    if(!_closure(self)->_function->_bgenerator) {
        return sq_throwerror(v, _SC("begin requires a generator-style function (contains 'yield')"));
    }

    SQInteger top = sq_gettop(v);
    SQInteger arg_last = top;
    ...
    if(top >= 2) {
        SQObjectType t = sq_gettype(v, top);
        if(t == OT_CLOSURE || t == OT_NATIVECLOSURE) {
            sq_getstackobj(v, top, &pending.callback);
            sq_addref(v, &pending.callback);
            pending.has_callback = SQTrue;
            arg_last = top - 1;
        }
    }
    ...
    v->Push(self);
    v->Push(v->_roottable);
    for(SQInteger i = 2; i <= arg_last; i++) {
        v->Push(stack_get(v, i));
    }

    SQInteger nparams = 1 + ((arg_last >= 2) ? (arg_last - 1) : 0);
    if(SQ_FAILED(sq_call(v, nparams, SQTrue, SQTrue))) {
        sq_settop(v, top);
        if(pending.has_callback) sq_release(v, &pending.callback);
        return SQ_ERROR;
    }
    ...
    if(sq_gettype(v, -1) != OT_GENERATOR) {
        sq_settop(v, top);
        if(pending.has_callback) sq_release(v, &pending.callback);
        return sq_throwerror(v, _SC("begin expected a generator result"));
    }
    ...
    SQInteger resume_top = sq_gettop(v);
    sq_pushobject(v, pending.generator);
    if(SQ_FAILED(sq_resume(v, SQTrue, SQTrue))) {
        sq_settop(v, resume_top);
        async_release_pending(v, pending);
        return SQ_ERROR;
    }
    sq_settop(v, resume_top);

    SQGenerator *gen = pending.generator._unVal.pGenerator;
    SQBool queued = SQFalse;
    if(gen->_state == SQGenerator::eDead) {
        if(pending.has_callback) {
            SQInteger cb_top = sq_gettop(v);
            sq_pushobject(v, pending.callback);
            sq_pushnull(v);
...
static SQInteger closure_end(HSQUIRRELVM v)
{
    SQObject &self = stack_get(v, 1);
    if(sq_type(self) != OT_CLOSURE) {
        return sq_throwerror(v, _SC("end can be called only on closures"));
    }

    SQClosure *cl = _closure(self);
    AsyncPendingMap::iterator it = g_async_pending.find(cl);
    if(it == g_async_pending.end() || it->second.empty()) {
        return sq_throwerror(v, _SC("end called with no pending begin invocation"));
    }

    AsyncPendingInvocation &pending = it->second.front();
    SQInteger resume_top = sq_gettop(v);
    sq_pushobject(v, pending.generator);
    if(SQ_FAILED(sq_resume(v, SQTrue, SQTrue))) {
        sq_settop(v, resume_top);
        async_release_pending(v, pending);
        it->second.pop_front();
        if(it->second.empty()) g_async_pending.erase(it);
        return SQ_ERROR;
    }
    sq_settop(v, resume_top);
...
const SQRegFunction SQSharedState::_closure_default_delegate_funcz[]={
    ...
    {_SC("begin"),closure_begin,-1, _SC("c")},
    {_SC("end"),closure_end,1, _SC("c")},
