...
public:
    ...
    void Statement(bool closeframe = true)
    {
        ...
        case TK_ASYNC:
            ...
            if(_token != TK_FUNCTION) {
                Error(_SC("expected 'function' after 'async'"));
            }
    ...
    /* EmitAsyncWrap — replaces the closure currently at TopTarget with
     * ::sqgi.__make_async(closure). Used after OP_CLOSURE for `async function`
     * literals so they self-launch into a Task. */
    void EmitAsyncWrap()
    {
        SQInteger inner = _fs->TopTarget();

        /* Build sqgi.__make_async in fresh slots above the closure. */
        _fs->AddInstruction(_OP_LOADROOT, _fs->PushTarget());
        _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
            _fs->GetConstant(_fs->CreateString(_SC("sqgi"))));
        Emit2ArgsOP(_OP_GET);
        _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
            _fs->GetConstant(_fs->CreateString(_SC("__make_async"))));
        Emit2ArgsOP(_OP_GET);
        SQInteger fn = _fs->TopTarget();

        /* Allocate contiguous slots for [this, arg]. */
        SQInteger this_slot = _fs->PushTarget();
        _fs->AddInstruction(_OP_LOADNULLS, this_slot, 1);
        SQInteger arg_slot = _fs->PushTarget();
        _fs->AddInstruction(_OP_MOVE, arg_slot, inner);
        ...
        _fs->AddInstruction(_OP_CALL, _fs->PushTarget(), fn, this_slot, 2);
    }
            ...
            case _SC('.'):
...

                {
                    ...
                    if(_token == TK_IDENTIFIER) {
                        member_id = Expect(TK_IDENTIFIER);
                    }
                    else if(_token > 255) {
                        /* Allow reserved words as property names: e.g. t.catch(...) */
                        const SQChar *kwname = _lex.Tok2Str(_token);
                        if(kwname) {
                            member_id = _fs->CreateString(kwname);
                            Lex();
                        }
                        else {
                            member_id = Expect(TK_IDENTIFIER); /* will raise */
                        }
                    }
                    else {
                        member_id = Expect(TK_IDENTIFIER); /* will raise */
                    }
                    _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(member_id));
                }
                if(_es.etype==BASE) {
                    Emit2ArgsOP(_OP_GET);
                    pos = _fs->TopTarget();
    ...
    SQInteger Factor()
    {
        ...
        case TK_AWAIT : Lex(); AwaitExp(); break;
    ...
    {
        if(_asynclevel <= 0) {
            Error(_SC("'await' can only be used inside async functions"));
        }

        _fs->AddInstruction(_OP_LOADROOT, _fs->PushTarget());
        _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->CreateString(_SC("sqgi"))));
        Emit2ArgsOP(_OP_GET);

        _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->CreateString(_SC("__await"))));
        Emit2ArgsOP(_OP_GET);
        ...
        for(SQInteger i = 0; i < 1; i++) _fs->PopTarget();
    ...
    void ParseTableOrClass(SQInteger separator,SQInteger terminator)
    {
        ...
        while(_token != terminator) {
            ...
            case TK_ASYNC: {
                ...
                CreateFunction(id, boundtarget, false, true);
                _fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, boundtarget);
                EmitAsyncWrap();
                             }
                             break;
    ...
    void FunctionStatement(bool is_async = false)
    {
        SQObject id;
        ...
        Expect(_SC('('));
        CreateFunction(id, boundtarget, false, is_async);
        _fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, boundtarget);
        if(is_async) EmitAsyncWrap();
        EmitDerefOp(_OP_NEWSLOT);
        _fs->PopTarget();
    }
    void ClassStatement()
    ...
    void FunctionExp(bool lambda = false, bool is_async = false)
    {
        Lex(); 
		SQInteger boundtarget = 0xFF;
		if (_token == _SC('[')) {
		...
		Expect(_SC('('));
        SQObjectPtr dummy;
        CreateFunction(dummy, boundtarget, lambda, is_async);
        _fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, boundtarget);
        if(is_async) EmitAsyncWrap();
    }
    void ClassExp()
    ...
    void CreateFunction(SQObject &name,SQInteger boundtarget,bool lambda = false,bool is_async = false)
    {
        ...
        SQInteger prev_async_level = _asynclevel;
        _fs = funcstate;
        _asynclevel = is_async ? 1 : 0;
        if(lambda) {
            Expression();
