/* C-DEL-01: native launcher for the manifest editor. No packaging logic. */
#include <gio/gio.h>
#ifdef G_OS_WIN32
#include <windows.h>
#endif
...
int main(int argc, char **argv)
{
...
#endif
    if (!self) { g_printerr("sqgipkg-gui: cannot locate executable\n"); return 1; }
    ...
    char *script = g_build_filename(share, "sqgipkg_gui", "main.nut", NULL);
    const char *selected_runtime = g_getenv("SQGI_GUI_RUNTIME");
#ifdef G_OS_WIN32
    char *runtime = selected_runtime ? g_strdup(selected_runtime) : g_build_filename(bin, "sqgi.exe", NULL);
#else
    char *runtime = selected_runtime ? g_strdup(selected_runtime) : g_build_filename(bin, "sqgi", NULL);
#endif
    g_setenv("SQGI_GUI_RUNTIME", runtime, TRUE);
    ...
    if (error) { g_printerr("sqgipkg-gui: %s\n", error->message); g_error_free(error); }
