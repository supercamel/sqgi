...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'shared-helper-tail-v29'; D=A/'constant-division-v30'; B=R/'build-jit-audit-20260909/constant-division-v30'; S=Path('/tmp/sqgi-constant-division-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-v30-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-v30-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-v29-x64-tests.log'); b=fails(Path('/tmp/sqgi-v30-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
previous=json.loads((P/'source-manifest.json').read_text()); previous.update(json.loads((A/'constant-division-v30-work-in-progress/additional-source-inputs.json').read_text())['files']); changed=[p for p,s in previous.items() if h(R/p)!=s]; assert len(changed)==8,changed
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v30')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-v30-{mode}-{enabled}-ctest-detail.log').read_text()
  assert '[coverage] executed 16 AArch64 generated-code groups' in detail
...
fault=json.loads(Path('/tmp/sqgi-v30-allocation-fault/verification.json').read_text());assert fault['verified'] and fault['cases']==13 and fault['engine_sha256']==h(Path('/tmp/sqgi-scalar-leaf-20260909/libsqgi.so.1'))
assert h(Path('/tmp/sqgi-v30-pilot/sqgi'))==h(Path('/tmp/sqgi-scalar-leaf-20260909/sqgi'))
for name in ['test-gates-verification.json','prior-contract-probes-verification.json','constant-probes-verification.json']:
...
for probe in ['buffer-probe','budget-preservation','tail-budget']:
 probes=json.loads(Path('/tmp/sqgi-v30-'+probe+'/verification.json').read_text());assert probes['verified']
pilots=json.loads((S/'pilot-verification.json').read_text());assert pilots['verified'] and pilots['no_regressions_over_5pct'] and pilots['candidate_sha256']==h(Path('/tmp/sqgi-v30-pilot/sqgi'))
...
manifest={p:h(R/p) for p in previous};assert len(manifest)==97
(D/'source-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');shutil.copy2(P/'unchanged-benchmarks-and-scripts.json',D)
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v30')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'constant-division-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v30-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-v30-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-v30-{mode}-holdouts'),D/f'{mode}-holdouts')
...
for name in ['libsqgi.so.1','sqgi_test_cpp_sqjit_side_exit','sqgi_test_cpp_sqjit_contracts','sqgi_test_cpp_sqjit_intrinsics','sqgi_test_cpp_sqjit_aarch64_emit','sqgi_test_cpp_sqjit_arithmetic','sqgi_test_cpp_sqjit_leaf']:
...
shutil.copytree('/tmp/sqgi-v30-allocation-fault',D/'allocation-fault')
for name in ['constant-budget','constant-proof','resource-retry']:
