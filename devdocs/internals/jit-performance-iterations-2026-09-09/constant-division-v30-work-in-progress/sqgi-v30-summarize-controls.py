...
S=Path('/tmp/sqgi-constant-division-study');R=Path('/home/sam/Programming/sqgi');out={}
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
candidate_hash=h(R/'build-jit-audit-20260909/constant-division-v30/sqgi')
for w in [4,5]:
 for group,count in [('kernel',15),('floats',7),('application',8),('application-seed17',8),('owners',10),('members',6),('methods',6)]:
  ...
  labels={'v29','v29_repeat','v30','v30_repeat'}| (set() if pilot else {'audit'})
  ...
  for k,all_m in d['medians'].items():
   ...
   row={'medians_ns':{n:v*1000 for n,v in m.items()},'ranges_ns':{n:[min(v)*1000,max(v)*1000] for n,v in samples.items()},'v30_over_v29':m['v30']/m['v29'],'repeat_over_v29':m['v30_repeat']/m['v29'],'repeat_over_v30':m['v30_repeat']/m['v30']}
   row['baseline_repeat_over_v29']=m['v29_repeat']/m['v29']
   row['paired_copies_ratio']=statistics.geometric_mean([m['v30'],m['v30_repeat']])/statistics.geometric_mean([m['v29'],m['v29_repeat']])
   if 'audit' in m:row['v30_over_audit']=m['v30']/m['audit']
   rows[k]=row
  out[f'{group}-w{w}']={'per_case':rows,'geomean_v30_over_v29':statistics.geometric_mean(v['v30_over_v29'] for v in rows.values()),'regressions_over_5pct_against_v29':[k for k,v in rows.items() if v['v30_over_v29']>1.05],'repeat_regressions_over_5pct_against_v29':[k for k,v in rows.items() if v['repeat_over_v29']>1.05]}
