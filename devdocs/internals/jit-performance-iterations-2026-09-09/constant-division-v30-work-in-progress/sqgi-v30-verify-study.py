...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';D=A/'constant-division-v30';S=Path('/tmp/sqgi-constant-division-study')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
binary=R/'build-jit-audit-20260909/constant-division-v30/sqgi';sha=h(binary)
assert sha=='e2d9658d4001b5711069d19f41d431875bf9e5387be06b2cd0b3b9f26d696728'
...
# No old iteration's regression-resolution exception applies here.
...
exit_code=json.loads((S/'native-code-comparison.json').read_text());assert len(code)==18
assert code['recycle']['before_bytes']==7344 and code['recycle']['after_bytes']==7328
assert code['rotate_buffers']['before_bytes']==7392 and code['rotate_buffers']['after_bytes']==7364
assert code['rotate_buffers']['shared_helper_layout']
for name in ['transfer','publish']:assert code[name]['normalized_equal']
reviews=json.loads((S/'arithmetic-code-review.json').read_text())
assert reviews['verified'] and reviews['binary_sha256']==sha
assert set(reviews['reviewed'])=={k for k,v in code.items() if v.get('needs_arithmetic_diff_review')}
