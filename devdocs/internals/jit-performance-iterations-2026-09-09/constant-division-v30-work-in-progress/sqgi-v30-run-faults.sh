#!/bin/bash
set -euo pipefail
cd /tmp/sqgi-v30-allocation-fault
...
for mode in exec strict-exec post-exec cold-exec shared-exec compact-exec; do
 LD_PRELOAD=/tmp/sqgi-v30-allocation-fault/fail_exec.so ./probe "$mode" > "$mode.log" 2>&1
done
...
print('13 V30 resource cases passed, including both helper tail forms')
PY
