...
for suite in ['kernels','applications','roots','members','owners','methods','floats']:
 ...
 profiles[suite]={'v29':before,'v30':after,'profile_sha256':h(S/'profiles'/(suite+'-flat.log'))}
...
for suite,p in profiles.items():
 lines += [f'## {suite}: leading V30 flat symbols','']
 for row in p['v30'][:8]:lines.append(f"- {row['percent']:.2f}% `{row['symbol']}` ({row['object']})")
 lines.append('')
lines += ['## Follow-up checks','','Constant positive divisors now use multiply/shift sequences in eligible','private and precise paths. Check the saved instruction counts against the','same-process native captures. Buffer helper count remains 1244 across four','successful native calls; arithmetic changes do not alter helper semantics.','Parent-loop/call coverage and private-path resource retry remain follow-ups.','Profile fractions are not speedup estimates.','']
