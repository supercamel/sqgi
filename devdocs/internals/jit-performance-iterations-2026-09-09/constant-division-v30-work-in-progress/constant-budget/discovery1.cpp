...
int main(int argc,char **argv)
{
    ...
    if((kind!="proto" && kind!="precise" && kind!="loop") || blocks<1 || blocks>120 || pad<0 || pad>100) return 2;
    const bool precise=kind=="precise",loop=kind=="loop";
    ...
    sqjit_observe_enable(v);int rc=0;
    {
        ...
        for(SQInteger ip=0;ip<p->_ninstructions;++ip) {
            ...
            if(i.op==_OP_LOADINT && i._arg1>=1515847680 && i._arg1<=1515847780) {
                if(loop) i._arg1=mutated ? 1 : 65536;
        ...
        if(_integer(*_table(record)->GetRawSlot(key))!=(precise ? 1 : 0)) return 7;
        ...
        printf("CASE %s instructions=%lld bytes=%lld high=%d divide=%d native=%lld result=%lld prefix=%d\n",
            ...
            (long long)(loop ? o.loop.successes : o.frame.successes),(long long)got,precise ? 1 : 0);
        v->temp_reg.Null();
    }
