...
int main(int argc,char **argv)
{
    ...
    if((kind!="proto" && kind!="precise" && kind!="loop" && kind!="loop_store") || blocks<1 || blocks>120 || pad<0 || pad>100) return 2;
    const bool precise=kind=="precise",loop=kind=="loop" || kind=="loop_store",store=kind=="loop_store";
    ...
    sqjit_observe_enable(v);int rc=0;
    {
        ...
        if(_integer(*_table(record)->GetRawSlot(key))!=(precise ? 1 : store ? 3 : 0)) return 7;
