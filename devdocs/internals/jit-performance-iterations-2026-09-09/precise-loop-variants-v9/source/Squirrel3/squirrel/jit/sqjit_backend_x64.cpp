...
static bool compile_loop(SQFunctionProto *proto, SQObjectPtr *entry_stack,
    ...
    SQJitCode *code, SQJitCompileAttempt &attempt)
{
    if(!proto || !entry_stack || !code || start_ip < 0 || header_ip < start_ip || end_ip < header_ip || end_ip >= proto->_ninstructions ||
    ...
    if(!code->Install(buf.bytes, buf.size))
        return attempt.Reject(-1, SQ_JIT_REJECT_RESOURCE, "executable memory installation failed");
    calls.RetainDependencies(*code);
    return true;
}


SQJitCompileResult sqjit_backend_compile_proto(SQFunctionProto *proto,
    SQObjectPtr *stack, SQClosure *closure, SQJitNative *native, bool /* allow_side_exits */)
{
