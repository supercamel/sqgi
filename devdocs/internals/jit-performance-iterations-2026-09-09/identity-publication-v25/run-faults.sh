#!/bin/bash
set -euo pipefail
cd /tmp/sqgi-v25-allocation-fault
./probe reference > reference.log 2>&1
./probe strict-reference > strict-reference.log 2>&1
LD_PRELOAD=/tmp/sqgi-v25-allocation-fault/fail_exec.so ./probe exec > exec.log 2>&1
LD_PRELOAD=/tmp/sqgi-v25-allocation-fault/fail_exec.so ./probe strict-exec > strict-exec.log 2>&1
gdb -q -batch -x inject.gdb ./probe > new.log 2>&1
python3 - <<'PY'
from pathlib import Path
import hashlib,json
p=Path('.');h=lambda f:hashlib.sha256(f.read_bytes()).hexdigest()
names=['reference.log','strict-reference.log','exec.log','strict-exec.log','new.log']
for n in names:
 s=(p/n).read_text();assert 'FAULT_PROBE_EXIT=0' in s,n
 if n in ['exec.log','strict-exec.log','new.log']:
 ...
 if n in ['exec.log','strict-exec.log']:assert 'EXEC_FAILURES=1 EXEC_ATTEMPTS=2' in s,n
...
print('V25 allocation failures verified against current engine')
PY
