...
bins={'audit':R/'build-jit-audit-20260909/baseline/sqgi','v23':R/'build-jit-audit-20260909/transactional-float-copies-v23/sqgi','v25':R/'build-jit-audit-20260909/identity-publication-v25/sqgi','v25_repeat':R/'build-jit-audit-20260909/identity-publication-v25/sqgi'}
...
for w in [5,4]:
 ...
 p=subprocess.run(command(bins['v25'],0),cwd=R,env=dict(env,SQGI_JIT='0'),capture_output=True,text=True,check=True,timeout=300);reference=parse(p.stdout);assert len(reference)==8
 ...
 metadata={'seed':17,'iterations':n,'warmups':w,'runs':5,'cpu':4,'hashes':{str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [script,*set(bins.values())]},'binaries':{k:str(v) for k,v in bins.items()},'purpose':'Check the effects of V25 precise built-ins and private publication against audit and accepted V23, retaining five rotating rounds, both warmup histories, and an identical candidate repeat. Controls do not replace or reweight the unchanged full56 and second-seed results.'}
 for rnd in range(5):
