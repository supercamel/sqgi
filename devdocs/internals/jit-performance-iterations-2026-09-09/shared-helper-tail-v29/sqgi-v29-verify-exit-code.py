...
import json,struct
S=Path('/tmp/sqgi-shared-helper-tail-study');out={}
paths=[]
...
for name,path,code in paths:
 ...
 tail=words[pos:end]
 if tail:assert len(tail)==6 and tail[0]&0xffc003ff==0xf94003e0 and tail[1:]==[0xaa1303e1,0xaa1503e2,0xaa1403e4,0xaa1603e5,0xd61f0200],(name,tail)
 out[name]={'mapped_bytes':len(raw),'code_bytes':end*4,'guard_count':len(found),'exact_resume_ips':found,'guard_offsets':incoming_map,'common_publication_offset':common*4,'shared_helper_tail_branch':bool(tail)}
