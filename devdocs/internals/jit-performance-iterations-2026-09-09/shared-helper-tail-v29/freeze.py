...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'shared-helper-v28'; D=A/'shared-helper-tail-v29'; B=R/'build-jit-audit-20260909/shared-helper-tail-v29'; S=Path('/tmp/sqgi-shared-helper-tail-study'); O=Path('/tmp/sqgi-profile-20260909/source')
...
a=fails(P/'sqgi-v28-x64-tests.log'); b=fails(Path('/tmp/sqgi-v29-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
...
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v29')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-v29-{mode}-{enabled}-ctest-detail.log').read_text()
...
fault=json.loads(Path('/tmp/sqgi-v29-allocation-fault/verification.json').read_text());assert fault['verified'] and fault['cases']==13 and fault['engine_sha256']==h(Path('/tmp/sqgi-scalar-leaf-20260909/libsqgi.so.1'))
assert h(Path('/tmp/sqgi-v29-pilot/sqgi'))==h(Path('/tmp/sqgi-scalar-leaf-20260909/sqgi'))
for probe in ['buffer-probe','budget-preservation','tail-budget']:
 probes=json.loads(Path('/tmp/sqgi-v29-'+probe+'/verification.json').read_text());assert probes['verified']
pilots=json.loads((S/'pilot-verification.json').read_text());assert pilots['verified'] and pilots['no_regressions_over_5pct'] and pilots['candidate_sha256']==h(Path('/tmp/sqgi-v29-pilot/sqgi'))
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v29')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'shared-helper-tail-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v29-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-v29-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-v29-{mode}-holdouts'),D/f'{mode}-holdouts')
