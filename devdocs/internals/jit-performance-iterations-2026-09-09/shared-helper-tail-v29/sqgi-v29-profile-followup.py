...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';S=Path('/tmp/sqgi-shared-helper-tail-study');P=A/'shared-helper-v28/study'
...
for suite in ['kernels','applications','roots','members','owners','methods','floats']:
 ...
 profiles[suite]={'v28':before,'v29':after,'profile_sha256':h(S/'profiles'/(suite+'-flat.log'))}
...
for suite,p in profiles.items():
 lines += [f'## {suite}: leading V29 flat symbols','']
 for row in p['v29'][:8]:lines.append(f"- {row['percent']:.2f}% `{row['symbol']}` ({row['object']})")
 lines.append('')
lines += ['## Follow-up checks','','The buffer helper count remains 1244 across four successful native calls;','the body is unchanged under pointer normalization and the shared tail is now','24 bytes. Inspect actual runtime changes and remaining helper/VM costs.','Parent-loop/call coverage remains a larger structural gap. No addresses from','different processes are joined, and PC fractions are not speedup estimates.','']
