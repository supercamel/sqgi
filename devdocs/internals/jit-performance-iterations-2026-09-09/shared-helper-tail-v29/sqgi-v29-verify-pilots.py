...
out={};expected_sha='24b4f8c1961001ffbad7a4735b998a912b87853f93bf71a10eab369a96ea9718'
for w in [5,4]:
 for suite,count in [('methods',6),('members',6),('owners',10)]:
  ...
  assert m['hashes'][m['commands']['v29'][0]]==expected_sha and m['commands']['v29']==m['commands']['v29_repeat']
  assert len(d['medians'])==count and len(d['raw'])==26
  ref=parse(d['raw'][0]['stdout']);assert len(ref)==count
  order=['v28','v29','v28_repeat','v29_repeat','node']
  ...
  out[p.stem]={'geomean':{n:statistics.geometric_mean(v[n] for v in ratios.values()) for n in ['v29','v29_repeat']},'per_case':ratios,'copy_diagnostics':copies,'regressions_over_5pct':{k:v for k,v in ratios.items() if max(v.values())>1.05},'result_sha256':h(p)}
