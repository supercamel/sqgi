...
study=/tmp/sqgi-precise-cache-proof-study
candidate=build-jit-audit-20260909/precise-cache-proofs-v17/sqgi
gdb -q -batch -x "$study/profiles/dump.gdb" "$candidate" > "$study/profiles/dump.log" 2>&1
for warmups in 5; do
for suite in owners methods members; do
 python3 tools/run_member_benchmarks.py --suite "$suite" --sqgi v16=build-jit-audit-20260909/precise-native-store-v16/sqgi --sqgi "v17=$candidate" --runs 5 --warmups "$warmups" --iterations 200000 --cpu 4 --output "$study/$suite-v16-v17-w${warmups}.json" > "$study/$suite-v16-v17-w${warmups}.log" 2>&1
done
done
...
python3 "$artifact/summarize_study.py" "$study" > /tmp/sqgi-precise-v17-study-summary.log
for warmups in 4; do
for suite in owners methods members; do
 python3 tools/run_member_benchmarks.py --suite "$suite" --sqgi v16=build-jit-audit-20260909/precise-native-store-v16/sqgi --sqgi "v17=$candidate" --runs 5 --warmups "$warmups" --iterations 200000 --cpu 4 --output "$study/$suite-v16-v17-w${warmups}.json" > "$study/$suite-v16-v17-w${warmups}.log" 2>&1
done
