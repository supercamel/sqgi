...
static int generated_copy_release()
{
    ...
    for(const SQChar *name : {_SC("copy_move"),_SC("copy_literal"),_SC("copy_integer"),_SC("copy_float"),_SC("copy_bool")}) {
        SQObjectPtr fn=named(v,name);SQFunctionProto *p=_closure(fn)->_function;
        const SQOpcode opcode[]={_OP_MOVE,_OP_LOAD,_OP_LOADINT,_OP_LOADFLOAT,_OP_LOADBOOL};
...
int main()
{
    int failures = generated_exits() + generated_exit_sites() + generated_store_release() + generated_cache_proofs() + generated_caches() + generated_scalars() + generated_cached_release() + generated_reference_reads() + generated_copy_release();
    SQVM *v = sq_open(64);
    SQJitContext &ctx = sqjit_context(v->_sharedstate);
