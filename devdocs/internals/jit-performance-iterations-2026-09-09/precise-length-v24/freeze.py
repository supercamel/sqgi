...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'transactional-float-copies-v23'; D=A/'precise-length-v24'; B=R/'build-jit-audit-20260909/precise-length-v24'; S=Path('/tmp/sqgi-precise-length-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-v24-callee-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-v24-callee-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-precise-v23-x64-tests.log'); b=fails(Path('/tmp/sqgi-v24-callee-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
previous=json.loads((P/'source-manifest.json').read_text()); changed=[p for p,s in previous.items() if h(R/p)!=s]; assert len(changed)==3,changed
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v24')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-v24-callee-{mode}-{enabled}-ctest-detail.log').read_text()
  assert '[coverage] executed 11 AArch64 generated-code groups' in detail
...
fault=json.loads(Path('/tmp/sqgi-v24-allocation-fault/verification.json').read_text());assert fault['verified'] and fault['engine_sha256']==h(Path('/tmp/sqgi-scalar-leaf-20260909/libsqgi.so.1'))
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v24')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'precise-length-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v24-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-v24-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-v24-callee-{mode}-holdouts'),D/f'{mode}-holdouts')
...
shutil.copytree('/tmp/sqgi-v24-allocation-fault',D/'allocation-fault')
(D/'README.md').write_text("# V24 frozen candidate — performance acceptance pending\n\nAll four 100-test runs and both 41-case holdout runs pass. Eight independent\nAArch64 coverage markers execute; x64 retains the same 32 known assertions.\nThe source and binary are frozen for profiling and five-round measurements.\nV23 remains the latest accepted timing result. See the sibling development\nsnapshot for the receiver-publication investigation and retained failed pilots.\n")
