...
out=pathlib.Path('/tmp/sqgi-precise-length-study/append-regression/candidate.json');ranges=[];bps=[];counts=collections.Counter();offsets=collections.Counter();entries=[];symbols={};captured_pid=0
...
run /tmp/sqgi-precise-length-study/append-regression/only-array_append.nut --bench --iterations=19 --warmups=0 --seed=17
python
