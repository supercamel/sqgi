...
local r={marker=0},a=[7],b=[11],live=b.weakref(),dead=[9].weakref();
for(local j=0;j<3;j++) {
 local source=j==0?b:(j==1?live:dead);
 local moved=route_copy_move(r,a,source);
 if(typeof moved!=(j==0?"array":"weakref") || moved!=source || route_copy_literal(r,a)!="copy-pinned" || route_copy_integer(r,a)!=1007 || route_copy_float(r,a)!=1.5 || route_copy_bool(r,a)!=true) throw "copy route mismatch";
}
