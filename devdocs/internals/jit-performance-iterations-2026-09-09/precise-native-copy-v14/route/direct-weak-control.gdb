...
start /tmp/sqgi-precise-native-copy-study/route/copy-direct-weak.nut
python
...
class Catch(gdb.Breakpoint):
    def stop(self):
        ...
        if name.startswith('route_'):
            ...
            hits.append(dict(name=name,ip=ip,op=int(inst['op']),dst=dst,source=int(inst['_arg1']),type=typ,refs=refs,source_type=str(gdb.parse_and_eval('(SQObjectPtr*)$x1')[int(inst['_arg1'])]['_type']) if int(inst['op'])==10 else None))
...
continue
python
(out/'direct-weak-v13-helper-hits.json').write_text(json.dumps(hits,indent=2)+'\n')
assert len(hits)==32,hits
moves=[x for x in hits if x['op']==10]
assert [x['source_type'] for x in moves]==['OT_ARRAY','OT_WEAKREF','OT_WEAKREF'],moves
extra=[x for x in hits if x['ip']==1]
assert len(extra)==2 and all(x['name']=='route_copy_literal' and x['op']==2 and x['dst']==4 and x['type']=='OT_TABLE' and x['refs']==3 for x in extra),extra
print('V13_DIRECT_WEAK_HELPER_HITS',len(hits))
end
