...
out = pathlib.Path('/tmp/sqgi-precise-native-copy-study/preflight')
seen = set()
class Capture(gdb.Breakpoint):
    ...
    def stop(self):
        try:
            ...
            if name not in ['recycle']: return False
            key=(name,self.mode)
...
run test/bench_member_owners.nut --iterations=23 --warmups=0
python
assert {name for name,mode in seen} == {'recycle'},seen
print('JIT_CAPTURE_VERIFIED owner canonical-frame function')
end
