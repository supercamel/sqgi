...
static int generated_length_intrinsics()
{
    ...
    CHECK(run(v,_SC(
        ...
        "function length_prepare(value,r,s,counter) { counter=counter+1; r.count=counter; return s.len()+r.seen; }\n"
    ...
    SQObjectPtr prepared=named(v,_SC("length_prepare"));
    SQFunctionProto *pp=_closure(prepared)->_function;bool receiver_alias=false;
    for(SQInteger ip=0;ip+1<pp->_ninstructions;++ip) {
        SQInstruction &i=pp->_instructions[ip],&next=pp->_instructions[ip+1];
        if(i.op==_OP_PREPCALLK && next.op==_OP_CALL && next._arg1==i._arg0 &&
            next._arg2==i._arg3 && next._arg3==1) {
            i._arg3=1;next._arg2=1;receiver_alias=true;
        }
    }
    ...
    CHECK(receiver_alias && warm_prepared(receivers[0]) && pp->_jit && pp->_jit->_entry &&
        pp->_jit->_entry->_requires_vm_frame,"prepare receiver alias uses actual precise code");
    for(const SQObjectPtr &receiver:receivers) for(int mode=0;mode<4;++mode) {
        ...
        for(int jit=0;jit<2;++jit) {
...
#endif
            {
                ...
                if(jit && (mode==0 || mode==3)) CHECK(after.frame.side_exits==before.frame.side_exits+1,
                    "observable final/ancestor-sensitive prepared drops take exactly one exit");
                if(jit && (mode==1 || mode==2)) CHECK(after.frame.successes==before.frame.successes+1 &&
