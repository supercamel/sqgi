...
static int generated_length_intrinsics()
{
    ...
    for(const SQChar *name : {_SC("length_prepare_legacy"),_SC("length_prepare_register"),_SC("length_prepare")}) {
    const bool legacy=scstrcmp(name,_SC("length_prepare"))!=0;
    ...
    for(SQInteger ip=0;ip+1<pp->_ninstructions;++ip) {
        ...
        if((i.op==_OP_PREPCALLK || i.op==_OP_PREPCALL) && next.op==_OP_CALL && next._arg1==i._arg0 &&
            next._arg2==i._arg3 && next._arg3==1) {
            i._arg3=1;next._arg2=1;receiver_alias=true;break;
        }
    }
