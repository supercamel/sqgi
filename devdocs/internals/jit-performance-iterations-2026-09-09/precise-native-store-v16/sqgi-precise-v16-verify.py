...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';D=A/'precise-native-store-v16';O=Path('/tmp/sqgi-profile-20260909/source')
...
with tempfile.TemporaryDirectory(prefix='sqgi-v16-final-reconstruction-') as td:
 t=Path(td)
 for p in paths:
...
assert [(v['suite'],v['name']) for v in s['regressions_over_5pct']]==[('kernels-seed17','array_append')]
for w in [4,5]:
 d=json.loads((D/f'kernel-control-w{w}.json').read_text());assert d['completed_rounds']==5 and len(d['medians'])==15
...
assert (D/'append-control-interpretation.md').exists()
assert len(json.loads((D/'array-diagnostic/diagnostic.json').read_text())['rows'])==30
...
shutil.copy2(__file__,D/'sqgi-precise-v16-verify.py')
files={R/p for p in old};files.update(p for p in D.rglob('*') if p.is_file());files.update(p for p in (R/'build-jit-audit-20260909/precise-native-store-v16').rglob('*') if p.is_file());files.update(R/p for p in changed)
