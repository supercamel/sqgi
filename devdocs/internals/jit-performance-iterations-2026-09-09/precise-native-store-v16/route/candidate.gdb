...
continue
python
(out/'candidate-hits.json').write_text(json.dumps(hits,indent=2)+'\n')
assert len(hits)==8,hits
targets=[h for h in hits if h['base']==2]
assert len(targets)==8 and all(h['owner_type'] in ['OT_TABLE','OT_INSTANCE'] for h in targets),targets
assert sum(h['source_type']=='OT_WEAKREF' for h in targets)==4,targets
print('STORE_HELPER_VERIFIED candidate 8')
end
