...
P=R/'devdocs/internals/jit-performance-iterations-2026-09-09/precise-shared-exit-v15/profiles'
D=Path('/tmp/sqgi-precise-native-store-study/profiles');result={}
for name in ['recycle','transfer','publish','table_apply','apply']:
 ...
 assert result[name]['native_heap_stores']==sum(c['op']==13 and (c['_arg0']==255 or c['_arg0']==c['_arg3']) for c in code),(name,result[name])
(D/'exit-code-verification.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: all guard branches select their exact bytecode IP and converge on one SIDE_EXIT publication/restoration; all eligible SETs have native heap publication instructions.')
