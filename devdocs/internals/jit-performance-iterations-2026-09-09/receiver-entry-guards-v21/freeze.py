...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'scalar-loop-entry-guards-v20'; D=A/'receiver-entry-guards-v21'; B=R/'build-jit-audit-20260909/receiver-entry-guards-v21'; S=Path('/tmp/sqgi-receiver-entry-guard-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-precise-v21-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-precise-v21-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-precise-v20-x64-tests.log'); b=fails(Path('/tmp/sqgi-precise-v21-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
...
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v21')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-precise-v21-{mode}-{enabled}-ctest-detail.log').read_text()
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v21')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'receiver-entry-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v21-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-precise-v21-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-precise-v21-{mode}-holdouts'),D/f'{mode}-holdouts')
