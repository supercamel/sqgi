...
start /tmp/sqgi-precise-native-reference-study/route/reference.nut
python
import gdb,json,pathlib
out=pathlib.Path('/tmp/sqgi-precise-native-reference-study/route')
hits=[]
...
continue
python
(out/'v12-helper-hits.json').write_text(json.dumps(hits,indent=2)+'\n')
assert len(hits)==36,hits
print('V12_CONTROL_HELPER_HITS',len(hits))
end
