...
r=Path('/tmp/sqgi-precise-native-reference-study')
result={'warmups4_speedup':{},'warmups5_speedup':{},'profitability':{},'profiles':{}}
for suite in ['owners','methods','members']:
 for w in [4,5]:
  d=json.loads((r/f'{suite}-v12-v13-w{w}.json').read_text());assert all(len(v)==5 for x in d['samples'].values() for v in x.values())
  v={k:m['v12']/m['v13'] for k,m in d['medians'].items()}
  result[f'warmups{w}_speedup'][suite]={'geomean_v12_v13':statistics.geometric_mean(v.values()),'per_case':v}
