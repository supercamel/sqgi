...
for w in [5,4]:
 ...
 p=subprocess.run(command(bins['v26'],0),cwd=R,env=dict(env,SQGI_JIT='0'),capture_output=True,text=True,check=True,timeout=300);reference=parse(p.stdout);assert len(reference)==15
