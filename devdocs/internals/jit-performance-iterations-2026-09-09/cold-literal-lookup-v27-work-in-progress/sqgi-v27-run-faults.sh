#!/bin/bash
set -euo pipefail
cd /tmp/sqgi-v27-allocation-fault
...
LD_PRELOAD=/tmp/sqgi-v27-allocation-fault/fail_exec.so ./probe exec > exec.log 2>&1
LD_PRELOAD=/tmp/sqgi-v27-allocation-fault/fail_exec.so ./probe strict-exec > strict-exec.log 2>&1
LD_PRELOAD=/tmp/sqgi-v27-allocation-fault/fail_exec.so ./probe post-exec > post-exec.log 2>&1
LD_PRELOAD=/tmp/sqgi-v27-allocation-fault/fail_exec.so ./probe cold-exec > cold-exec.log 2>&1
