...
import json,struct
D=Path('/tmp/sqgi-precise-integer-compare-study/call-regression-captures/extra')
...
for key,entry in meta['candidate'].items():
 ...
 assert same and (bool(removed) if key!='array_append_kernel-entry' else not removed),(key,result[key])
