...
class Catch(gdb.Breakpoint):
 def stop(self):
  ...
  return False
breakpoints=[Catch('*'+str(int(gdb.parse_and_eval('&'+symbol))),internal=True) for symbol in json.loads((out/'candidate-symbols.json').read_text())]
...
print('COMPARISON_HELPERS_VERIFIED candidate 4')
end
