...
out = pathlib.Path('/tmp/sqgi-precise-integer-compare-study/route/compact')
seen = set()
class Capture(gdb.Breakpoint):
    ...
    def stop(self):
        try:
            ...
            if name not in ['exit_sites','cached_many']: return False
            key=(name,self.mode)
...
continue
python
assert {name for name,mode in seen} == {'exit_sites','cached_many'},seen
print('JIT_CAPTURE_VERIFIED two compact canonical-frame functions')
end
