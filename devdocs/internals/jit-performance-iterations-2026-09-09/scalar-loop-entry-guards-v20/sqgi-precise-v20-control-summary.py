...
S=Path('/tmp/sqgi-scalar-entry-guard-study');result={}
for w in [4,5]:
 for group in ['kernel','floats']:
  ...
  for k,m in d['medians'].items():
   per[k]={'medians_ns':{name:value*1000 for name,value in m.items()},'v20_over_audit':m['v20']/m['audit'],'v20_over_v19':m['v20']/m['v19'],'identical_repeat_over_v20':m['v20_repeat']/m['v20']}
  result[f'{group}-w{w}']={'per_case':per,'regressions_over_5pct_against_v19':[k for k,v in per.items() if v['v20_over_v19']>1.05],'geomean_v20_over_v19':statistics.geometric_mean(v['v20_over_v19'] for v in per.values())}
