...
study=/tmp/sqgi-scalar-entry-guard-study
candidate=build-jit-audit-20260909/scalar-loop-entry-guards-v20/sqgi
for warmups in 5; do
for suite in owners methods members; do
 python3 tools/run_member_benchmarks.py --suite "$suite" --sqgi v19=build-jit-audit-20260909/closure-witness-guards-v19/sqgi --sqgi "v20=$candidate" --runs 5 --warmups "$warmups" --iterations 200000 --cpu 4 --output "$study/$suite-v19-v20-w${warmups}.json" > "$study/$suite-v19-v20-w${warmups}.log" 2>&1
done
done
...
python3 "$artifact/summarize_study.py" "$study" > /tmp/sqgi-precise-v20-study-summary.log
for warmups in 4; do
for suite in owners methods members; do
 python3 tools/run_member_benchmarks.py --suite "$suite" --sqgi v19=build-jit-audit-20260909/closure-witness-guards-v19/sqgi --sqgi "v20=$candidate" --runs 5 --warmups "$warmups" --iterations 200000 --cpu 4 --output "$study/$suite-v19-v20-w${warmups}.json" > "$study/$suite-v19-v20-w${warmups}.log" 2>&1
done
...
python3 /tmp/sqgi-precise-v20-kernel-control.py > /tmp/sqgi-precise-v20-kernel-control.log 2>&1
for warmups in 5 4; do
 python3 tools/run_member_benchmarks.py --suite floats --sqgi audit=build-jit-audit-20260909/baseline/sqgi --sqgi v19=build-jit-audit-20260909/closure-witness-guards-v19/sqgi --sqgi "v20=$candidate" --sqgi "v20_repeat=$candidate" --runs 5 --warmups "$warmups" --iterations 200000 --cpu 4 --output "$study/floats-control-w${warmups}.json" > "$study/floats-control-w${warmups}.log" 2>&1
done
...
for label in control candidate; do
 if [ "$label" = control ]; then binary=build-jit-audit-20260909/closure-witness-guards-v19/sqgi; else binary="$candidate"; fi
