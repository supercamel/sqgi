...
r=Path('/tmp/sqgi-closure-witness-guard-study')
result={'warmups4_speedup':{},'warmups5_speedup':{},'profitability':{},'profiles':{}}
for suite in ['owners','methods','members']:
 for w in [4,5]:
  d=json.loads((r/f'{suite}-v18-v19-w{w}.json').read_text());assert all(len(v)==5 for x in d['samples'].values() for v in x.values())
  v={k:m['v18']/m['v19'] for k,m in d['medians'].items()}
  result[f'warmups{w}_speedup'][suite]={'geomean_v18_v19':statistics.geometric_mean(v.values()),'per_case':v}
