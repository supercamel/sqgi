...
S=Path('/tmp/sqgi-closure-witness-guard-study');result={}
for w in [4,5]:
 for group in ['kernel','floats']:
  ...
  for k,m in d['medians'].items():
   per[k]={'medians_ns':{name:value*1000 for name,value in m.items()},'v19_over_audit':m['v19']/m['audit'],'v19_over_v18':m['v19']/m['v18'],'identical_repeat_over_v19':m['v19_repeat']/m['v19']}
  result[f'{group}-w{w}']={'per_case':per,'regressions_over_5pct_against_v18':[k for k,v in per.items() if v['v19_over_v18']>1.05],'geomean_v19_over_v18':statistics.geometric_mean(v['v19_over_v18'] for v in per.values())}
