...
import json,struct
D=Path('/tmp/sqgi-closure-witness-guard-study/call-regression-captures/extra')
...
for key,entry in meta['candidate'].items():
 old=words('candidate',key);new=words('control',key);weak={v['address']:v for v in meta['control'][key]['weak_witnesses'] if v['type'] in ['OT_CLOSURE','OT_NATIVECLOSURE']};removed=[];kept=[];pos=0
 while pos<len(new):
 ...
 result[key]=dict(weak_guard_sites=removed,removed_instructions=sum(v['instructions'] for v in removed),normalized_rest_matches=same,old_instructions=len(new),new_instructions=len(old))
 assert bool(removed)==(key!='array_append_kernel-entry') and same,(key,result[key])
 # The candidate passes the retained weak object in the existing expected-identity argument.
 candidate_weak={v['address'] for v in entry['weak_witnesses'] if v['type'] in ['OT_CLOSURE','OT_NATIVECLOSURE']}
 passed=[mov(old,i,r,64)[0] for i in range(len(old)) for r in [2,3] if mov(old,i,r,64) and mov(old,i,r,64)[0] in candidate_weak]
 assert len(passed)==len(removed),(key,passed,removed)
 result[key]['weak_witness_arguments']=passed
(D/'code-comparison.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2));print('PASS: redundant emitted lifetime guards removed; retained witnesses passed to existing identity helpers; remaining streams match after masking relocated wide constants and branch offsets.')
