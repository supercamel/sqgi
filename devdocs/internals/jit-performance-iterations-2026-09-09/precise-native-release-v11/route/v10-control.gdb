...
start /tmp/sqgi-precise-native-release-study/route/owned.nut
python
import gdb,json,pathlib
out=pathlib.Path('/tmp/sqgi-precise-native-release-study/route')
hits=[]
...
continue
python
(out/'v10-helper-hits.json').write_text(json.dumps(hits,indent=2)+'\n')
assert len(hits)>0,hits
print('V10_CONTROL_HELPER_HITS',len(hits))
end
