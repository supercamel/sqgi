...
for w in [5,4]:
 ...
 metadata={'seed':83,'iterations':n,'warmups':w,'runs':5,'cpu':4,'hashes':{str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [script,*set(bins.values())]},'binaries':{k:str(v) for k,v in bins.items()},'purpose':'Investigate the second-seed records_4096 slowdown using the unchanged full eight-application script, five rotating rounds, both warmup histories, and an identical candidate repeat. Diagnostic controls do not replace or reweight the full56 or second-seed measurements.'}
 for rnd in range(5):
