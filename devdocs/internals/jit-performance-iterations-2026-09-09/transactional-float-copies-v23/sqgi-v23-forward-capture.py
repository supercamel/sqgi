...
S=Path('/tmp/sqgi-transactional-float-copies-study/forwarding-captures');S.mkdir(exist_ok=True)
source=Path('/home/sam/Programming/sqgi/Squirrel3/squirrel/jit/sqjit_backend_aarch64_emit.cpp')
line=next(i for i,s in enumerate(source.read_text().splitlines(),1) if s.strip()=='*buf=output;')
...
out=pathlib.Path('OUT');events=[];active={};installed={};finishes=[]
def readbuf(address):
...
class Commit(gdb.Breakpoint):
 def stop(self):
  f=gdb.newest_frame();pid=gdb.selected_inferior().pid;e=next(e for e in reversed(events) if e['pid']==pid and 'changed' not in e)
...
commit=Commit('SOURCE:LINE',internal=True)
...
'''.replace('OUT',str(S)).replace('SOURCE:LINE',str(source)+':'+str(line))
(S/'capture.gdb').write_text(g)
