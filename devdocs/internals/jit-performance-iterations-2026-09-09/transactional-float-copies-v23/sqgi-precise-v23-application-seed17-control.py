...
for w in [5,4]:
 n=200000;out=S/f'application-seed17-control-w{w}.json';assert not out.exists(),out
 def command(binary,warmups=w):return ['taskset','-c','4',str(binary),str(script),'--bench',f'--iterations={n}',f'--warmups={warmups}','--seed=17']
 p=subprocess.run(command(bins['v23'],0),cwd=R,env=dict(env,SQGI_JIT='0'),capture_output=True,text=True,check=True,timeout=300);reference=parse(p.stdout);assert len(reference)==8
 ...
 metadata={'seed':17,'iterations':n,'warmups':w,'runs':5,'cpu':4,'hashes':{str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [script,*set(bins.values())]},'binaries':{k:str(v) for k,v in bins.items()},'purpose':'Investigate the first-seed filter_stream slowdown using the unchanged full eight-application script, five rotating rounds, both warmup histories, and an identical candidate repeat. Diagnostic controls do not replace or reweight the full56 or second-seed measurements.'}
 for rnd in range(5):
