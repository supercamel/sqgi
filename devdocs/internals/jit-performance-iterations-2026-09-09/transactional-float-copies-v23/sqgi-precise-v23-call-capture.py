...
s=Path('/tmp/sqgi-transactional-float-copies-study');d=s/'call-regression-captures';d.mkdir(exist_ok=True)
for label in ['control','candidate']:
 p=d/label;p.mkdir(exist_ok=True)
...
out=pathlib.Path('OUT');seen=set();metadata={}
class Capture(gdb.Breakpoint):
 ...
 def stop(self):
  ...
  if name not in ['direct_call_kernel','float_call_kernel','channels','pressure','retained']:return False
  key=name+'-'+self.mode
  ...
  metadata[key]=dict(mapped_bytes=size,weak_witnesses=witnesses)
...
assert all(any(k.startswith(name+'-') for k in seen) for name in ['direct_call_kernel','float_call_kernel','channels','pressure','retained']),seen
