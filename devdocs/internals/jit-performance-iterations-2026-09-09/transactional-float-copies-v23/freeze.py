...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'local-slot-forwarding-v22'; D=A/'transactional-float-copies-v23'; B=R/'build-jit-audit-20260909/transactional-float-copies-v23'; S=Path('/tmp/sqgi-transactional-float-copies-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-precise-v23-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-precise-v23-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-precise-v22-x64-tests.log'); b=fails(Path('/tmp/sqgi-precise-v23-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
previous=json.loads((P/'source-manifest.json').read_text()); changed=[p for p,s in previous.items() if h(R/p)!=s]; assert len(changed)==4,changed
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v23')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-precise-v23-{mode}-{enabled}-ctest-detail.log').read_text()
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v23')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'typed-copy-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v23-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-precise-v23-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-precise-v23-{mode}-holdouts'),D/f'{mode}-holdouts')
