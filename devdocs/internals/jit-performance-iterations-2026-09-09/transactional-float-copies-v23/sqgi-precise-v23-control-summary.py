...
for w in [4,5]:
 for group in ['kernel','floats']:
  ...
  for k,m in d['medians'].items():
   per[k]={'medians_ns':{name:value*1000 for name,value in m.items()},'v23_over_audit':m['v23']/m['audit'],'v23_over_v22':m['v23']/m['v22'],'identical_repeat_over_v23':m['v23_repeat']/m['v23']}
  result[f'{group}-w{w}']={'per_case':per,'regressions_over_5pct_against_v22':[k for k,v in per.items() if v['v23_over_v22']>1.05],'geomean_v23_over_v22':statistics.geometric_mean(v['v23_over_v22'] for v in per.values())}
