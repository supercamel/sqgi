...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';D=A/'transactional-float-copies-v23';B=R/'build-jit-audit-20260909/transactional-float-copies-v23';O=Path('/tmp/sqgi-profile-20260909/source')
...
with tempfile.TemporaryDirectory(prefix='sqgi-v23-final-reconstruct-') as td:
 t=Path(td)
 for p in changed:
...
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  log=(D/f'sqgi-precise-v23-{mode}-{enabled}-tests.log').read_text();assert '100% tests passed, 0 tests failed out of 100' in log
  detail=(D/f'sqgi-precise-v23-{mode}-{enabled}-ctest-detail.log').read_text()
  for marker in ['[coverage] executed 10 AArch64 generated-code groups','[coverage] executed AArch64 loop adaptation and lifetime contracts','[coverage] executed AArch64 native closure lifetime contract','[coverage] executed AArch64 scalar loop entry guard contracts','[coverage] executed AArch64 receiver publication contracts (scalar, final, nonfinal, ancestors)','[coverage] executed AArch64 local-slot forwarding contracts (bits, calls, branches, eviction, aliases)','[coverage] executed AArch64 typed-copy forwarding and fallback contracts']:assert marker in detail
...
x=fails(D/'sqgi-precise-v23-x64-tests.log');assert sum(x.values())==32 and x==fails(A/'local-slot-forwarding-v22/sqgi-precise-v22-x64-tests.log')
...
for w in [4,5]:
 for suite in ['owners','methods','members']:
  d=json.loads((D/f'{suite}-v22-v23-w{w}.json').read_text());assert all(len(v)==5 for x in d['samples'].values() for v in x.values())
...
gap=json.loads((D/'coverage-gaps/summary.json').read_text());assert gap['bytecodes']==53 and gap['stack_slots']==12 and gap['consecutive_default_length_candidate_ips']==[25,26]
for label,engine in [('candidate',B/'native-validation'),('v22',R/'build-jit-audit-20260909/local-slot-forwarding-v22/native-validation')]:
...
assert 'four-file V22 delta' in (D/'README.md').read_text()
assert (B/'native-validation/sqgi_test_cpp_sqjit_aarch64_emit').exists()
print('VERIFIED V23: complete selected gates, unchanged benchmarks, exact branch/instruction/installed-code checks, retained lifetime regressions, full fresh timings and profiles. Goal remains active.')
