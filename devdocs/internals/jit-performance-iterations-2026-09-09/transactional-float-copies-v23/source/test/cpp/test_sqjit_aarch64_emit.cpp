...
static int forwarding_encoding_tests()
{
    ...
    // Recorded GP copies must survive refusal, including a late output overflow.
...
static int forwarding_native_tests()
{
    ...
    for(int mode=0;mode<10;++mode) {
        ...
        SQInteger target=-1,copy_offset=-1;
        if(mode==0) {
        ...
        CHECK(sqjit_a64_forward_local_slots(&forwarded,32,mode==9 ? &copy_offset : nullptr,mode==9 ? 1 : 0),"synthetic native contract actually forwards slots");
        for(int enabled=0;enabled<2;++enabled) {
            ...
            if(executable) for(SQUnsignedInteger input : {SQUnsignedInteger(0),SQUnsignedInteger(1),SQUnsignedInteger(0x8000000000000000ULL),SQUnsignedInteger(0x7ff8000000001234ULL),SQUnsignedInteger(0x7ff0000000000001ULL),SQUnsignedInteger(0xffffffffffffffffULL)}) {
                ...
                if(mode==0 || mode==9) CHECK(output[0]==input,"mixed integer/float copies preserve zero sign and NaN payload bits");
                ...
                if(mode==5) CHECK(output[0]==77,"derived private-frame alias invalidates forwarded values");
