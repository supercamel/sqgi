...
s=Path('/tmp/sqgi-transactional-float-copies-study');d=s/'call-regression-captures/extra';d.mkdir(exist_ok=True)
...
class Capture(gdb.Breakpoint):
 ...
 def stop(self):
  ...
  if name not in ['branch_call_kernel','array_append_kernel','filter']:return False
  key=name+'-'+self.mode
...
assert all(any(k.startswith(name+'-') for k in seen) for name in ['branch_call_kernel','array_append_kernel','filter']),seen
