...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';D=A/'transactional-float-copies-v23';B=R/'build-jit-audit-20260909/transactional-float-copies-v23';S=Path('/tmp/sqgi-transactional-float-copies-study')
...
for glob in ['sqgi-precise-v23-*','sqgi-v23-*.py','sqgi-freeze-v23.py']:
...
for source,name in [('libsqgi.so.1.0.0','libsqgi.so.1')]+[(f'sqgi_test_cpp_sqjit_{s}',f'sqgi_test_cpp_sqjit_{s}') for s in ['side_exit','contracts','intrinsics','aarch64_emit']]:
...
print('Archived V23; verified',len(old),'prior hashes. Reports and global manifest pending.')
