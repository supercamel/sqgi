...
for folder in ['call-regression-captures','call-regression-captures/extra']:
 ...
 for name in meta['control']:
  ...
  assert list(map(norm,old))==list(map(norm,new)),(name,'previous forwarded artifact changed')
  ...
  result[name]=dict(control_code_bytes=len(old)*4,candidate_code_bytes=len(new)*4,scratch_moves=scratch,whole_artifact_unchanged=True,entry_guard_preserved=name!='array_append_kernel-entry',array_artifact_unchanged=name=='array_append_kernel-entry')
assert len(result)==8
