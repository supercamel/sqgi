...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'cold-literal-lookup-v27'; D=A/'shared-helper-v28'; B=R/'build-jit-audit-20260909/shared-helper-v28'; S=Path('/tmp/sqgi-shared-helper-study'); O=Path('/tmp/sqgi-profile-20260909/source')
...
a=fails(P/'sqgi-v27-x64-tests.log'); b=fails(Path('/tmp/sqgi-v28-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
...
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v28')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
...
fault=json.loads(Path('/tmp/sqgi-v28-allocation-fault/verification.json').read_text());assert fault['verified'] and fault['engine_sha256']==h(Path('/tmp/sqgi-scalar-leaf-20260909/libsqgi.so.1'))
assert h(Path('/tmp/sqgi-v28-pilot/sqgi'))==h(Path('/tmp/sqgi-scalar-leaf-20260909/sqgi'))
for probe in ['buffer-probe','budget-preservation','tail-budget']:
 probes=json.loads(Path('/tmp/sqgi-v28-'+probe+'/verification.json').read_text());assert probes['verified']
pilots=json.loads((S/'pilot-verification.json').read_text());assert pilots['verified'] and pilots['no_regressions_over_5pct'] and pilots['candidate_sha256']==h(Path('/tmp/sqgi-v28-pilot/sqgi'))
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v28')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'shared-helper-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v28-reconstruct-') as td:
 t=Path(td)
...
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-v28-{mode}-holdouts'),D/f'{mode}-holdouts')
...
shutil.copytree('/tmp/sqgi-v28-allocation-fault',D/'allocation-fault')
(D/'README.md').write_text("# V28 frozen shared-helper candidate — performance acceptance pending\n\nAll four 100-test runs and both 41-case holdout runs pass. Twelve independent\nAArch64 coverage markers execute; x64 retains the same 32 known assertions.\nThe source and binary are frozen for full profiling and five-round measurements.\nV27 remains the latest accepted full performance result.\n")
