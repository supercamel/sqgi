...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';S=Path('/tmp/sqgi-shared-helper-study');P=A/'cold-literal-lookup-v27/study'
...
for suite in ['kernels','applications','roots','members','owners','methods','floats']:
 ...
 profiles[suite]={'v27':before,'v28':after,'profile_sha256':h(S/'profiles'/(suite+'-flat.log'))}
...
for suite,p in profiles.items():
 lines += [f'## {suite}: leading V28 flat symbols','']
 for row in p['v28'][:8]:lines.append(f"- {row['percent']:.2f}% `{row['symbol']}` ({row['object']})")
 lines.append('')
lines += ['## Follow-up checks','','The buffer probe confirms shared tier3 code and 528 fewer helper calls across','four successful native invocations. Inspect remaining helper and VM costs;','the shared trampoline may support an ABI-preserving tail branch as a separate','measured follow-up. Methods still need parent-loop/call overhead work, and','owner DLOAD remains a separate lead. No cross-process address join is used.','']
