...
p=Path('/tmp/sqgi-v28-tail-budget');d=json.loads((p/'metadata.json').read_text());witnesses=[]
...
for n in range(1,25):
 ...
 assert c['successful_emission']['tier']==(6 if n<=10 else 4)
 tail=tails[-1];size=tail['body_bytes']+40;raw=(p/(name+'.bin')).read_bytes()
 assert size<=8192 and len(raw)==c['mapped_bytes']
 words=struct.unpack('<10I',raw[tail['body_bytes']:size])
 assert words[0:2]==(0xa9bf7bfd,0x910003fd)
 assert words[-3:]==(0xd63f0200,0xa8c17bfd,0xd65f03c0)
 assert tail['attempt']==len(d['attempts'][name])-1
 for t in tails[:-1]:
  assert t['body_bytes']<=8192 and t['body_bytes']+40>8192
...
assert d['tails']['budget_pad_10'][0]['body_bytes']+40==8192
assert d['tails']['budget_pad_11'][0]['body_bytes']+40==8196
...
print('24 native fixtures; 20 tail-only overflow retries verified')
