...
out={};expected_sha='ef6bbeab6b37097be7e2314d6f71067f797ce34bdb40a9c0ddc8454bbd608330'
for w in [5,4]:
 for suite,count in [('methods',6),('members',6),('owners',10)]:
  ...
  assert m['hashes'][m['commands']['v28'][0]]==expected_sha and m['commands']['v28']==m['commands']['v28_repeat']
  assert len(d['medians'])==count and len(d['raw'])==21
  ref=parse(d['raw'][0]['stdout']);assert len(ref)==count
  order=['v27','v28','v28_repeat','node']
  ...
  for k,m in d['medians'].items():
   ...
   ratios[k]={n:m[n]/m['v27'] for n in ['v28','v28_repeat']}
  out[p.stem]={'geomean':{n:statistics.geometric_mean(v[n] for v in ratios.values()) for n in ['v28','v28_repeat']},'per_case':ratios,'regressions_over_5pct':{k:v for k,v in ratios.items() if max(v.values())>1.05},'result_sha256':h(p)}
