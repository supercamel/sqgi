...
class Returned(gdb.Breakpoint):
 ...
 def stop(self):
  ...
  if len(returns)==4:
   ...
   assert metadata['perf_exit_code'] in (0,-signal.SIGINT),metadata
  return False
class Enter(gdb.Breakpoint):
 ...
 def stop(self):
  ...
  return False
class Compiled(gdb.Breakpoint):
 def __init__(self):super().__init__('/home/sam/Programming/sqgi/Squirrel3/squirrel/jit/sqjit.cpp:610',internal=True)
 def stop(self):
  proto=gdb.parse_and_eval('proto').dereference()
  if proto_name(proto)!='rotate_buffers':return False
  self.enabled=False
  native=proto['_jit'].dereference()['_entry'].dereference();code=native['_code'];address=int(code['entry']);size=int(code['size']);pid=gdb.selected_inferior().pid
...
class Compile(gdb.Breakpoint):
 def stop(self):
  ...
  keepers.append(Compiled());self.enabled=False;return False
...
assert metadata['perf_exit_code'] in (0,-signal.SIGINT)
