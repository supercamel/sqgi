...
out=pathlib.Path('/tmp/sqgi-v26-method-probe-complete');captures={};counts=collections.Counter();entries=collections.Counter();breakpoints=[];sites={}
def pname(p):
...
class Capture(gdb.Breakpoint):
 def stop(self):
  ...
  if not captures:
   for op,symbol in [(14,'&sqjit_a64_precise_step<(SQOpcode)14, true>'),(9,'&sqjit_a64_precise_step<(SQOpcode)9>'),(13,'&sqjit_a64_precise_step<(SQOpcode)13, true>')]:
