...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'identity-publication-v25'; D=A/'owning-postincrement-v26'; B=R/'build-jit-audit-20260909/owning-postincrement-v26'; S=Path('/tmp/sqgi-owning-postincrement-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-v26-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-v26-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-v25-x64-tests.log'); b=fails(Path('/tmp/sqgi-v26-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
...
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v26')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-v26-{mode}-{enabled}-ctest-detail.log').read_text()
  assert '[coverage] executed 13 AArch64 generated-code groups' in detail
...
fault=json.loads(Path('/tmp/sqgi-v26-allocation-fault/verification.json').read_text());assert fault['verified'] and fault['engine_sha256']==h(Path('/tmp/sqgi-scalar-leaf-20260909/libsqgi.so.1'))
assert h(Path('/tmp/sqgi-v26-pilot/sqgi'))==h(Path('/tmp/sqgi-scalar-leaf-20260909/sqgi'))
probes=json.loads(Path('/tmp/sqgi-v26-probes/verification.json').read_text());assert probes['verified']
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v26')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'owning-postincrement-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v26-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-v26-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-v26-{mode}-holdouts'),D/f'{mode}-holdouts')
...
shutil.copytree('/tmp/sqgi-v26-allocation-fault',D/'allocation-fault')
(D/'README.md').write_text("# V26 frozen candidate — performance acceptance pending\n\nAll four 100-test runs and both 41-case holdout runs pass. Ten independent\nAArch64 coverage markers execute; x64 retains the same 32 known assertions.\nThe source and binary are frozen for full profiling and five-round measurements.\nV25 remains the latest accepted full performance result.\n")
