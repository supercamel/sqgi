...
for label in control candidate; do
 if [ "$label" = control ]; then binary=build-jit-audit-20260909/identity-publication-v25/sqgi; else binary="$candidate"; fi
...
python3 /tmp/sqgi-v26-verify-exit-code.py > /tmp/sqgi-v26-exit-code.log
python3 /tmp/sqgi-v26-compare-code.py > /tmp/sqgi-v26-code-comparison.log
