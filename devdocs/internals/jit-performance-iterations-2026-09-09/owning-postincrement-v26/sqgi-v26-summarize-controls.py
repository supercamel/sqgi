...
S=Path('/tmp/sqgi-owning-postincrement-study');R=Path('/home/sam/Programming/sqgi');out={}
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
candidate_hash=h(R/'build-jit-audit-20260909/owning-postincrement-v26/sqgi')
for w in [4,5]:
 for group,count in [('kernel',15),('floats',7),('application',8),('application-seed17',8),('owners',10),('members',6),('methods',6)]:
  pilot=group in ['owners','members'];member=group in ['floats','owners','members','methods']
  ...
  labels={'v25','v26','v26_repeat'}| (set() if pilot else {'audit'})
  ...
  for k,all_m in d['medians'].items():
   ...
   row={'medians_ns':{n:v*1000 for n,v in m.items()},'ranges_ns':{n:[min(v)*1000,max(v)*1000] for n,v in samples.items()},'v26_over_v25':m['v26']/m['v25'],'repeat_over_v25':m['v26_repeat']/m['v25'],'repeat_over_v26':m['v26_repeat']/m['v26']}
   if 'audit' in m:row['v26_over_audit']=m['v26']/m['audit']
   rows[k]=row
  out[f'{group}-w{w}']={'per_case':rows,'geomean_v26_over_v25':statistics.geometric_mean(v['v26_over_v25'] for v in rows.values()),'regressions_over_5pct_against_v25':[k for k,v in rows.items() if v['v26_over_v25']>1.05],'repeat_regressions_over_5pct_against_v25':[k for k,v in rows.items() if v['repeat_over_v25']>1.05]}
