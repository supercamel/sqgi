...
out=pathlib.Path('/tmp/sqgi-v26-application-call-probe');captures={};entries=collections.Counter();returns=collections.Counter();helpers=collections.Counter();breakpoints=[]
wanted={'route_events','pricing_events','filter_stream'}
def pname(p):
...
class Returned(gdb.Breakpoint):
 def __init__(self,address,name):super().__init__('*'+str(address),temporary=True,internal=True);self.name=name
 def stop(self):returns[self.name+':'+str(int(gdb.parse_and_eval('$x0')))]+=1;return False
class Enter(gdb.Breakpoint):
 def __init__(self,address,name):super().__init__('*'+str(address),internal=True);self.name=name
 def stop(self):
  entries[self.name]+=1;breakpoints.append(Returned(int(gdb.parse_and_eval('$x30')),self.name));return False
