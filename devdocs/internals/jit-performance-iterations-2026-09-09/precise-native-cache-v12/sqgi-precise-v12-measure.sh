...
study=/tmp/sqgi-precise-native-cache-study
candidate=build-jit-audit-20260909/precise-native-cache-v12/sqgi
...
python3 "$artifact/summarize_study.py" "$study" > /tmp/sqgi-precise-v12-study-summary.log
for suite in owners methods members; do
 python3 tools/run_member_benchmarks.py --suite "$suite" --sqgi v11=build-jit-audit-20260909/precise-native-release-v11/sqgi --sqgi "v12=$candidate" --runs 5 --warmups 4 --iterations 200000 --cpu 4 --output "$study/$suite-v11-v12-w4.json" > "$study/$suite-v11-v12-w4.log" 2>&1
done
