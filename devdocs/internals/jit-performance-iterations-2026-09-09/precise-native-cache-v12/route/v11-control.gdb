...
start /tmp/sqgi-precise-native-cache-study/route/cache.nut
python
import gdb,json,pathlib
out=pathlib.Path('/tmp/sqgi-precise-native-cache-study/route')
hits=[]
...
continue
python
(out/'v11-helper-hits.json').write_text(json.dumps(hits,indent=2)+'\n')
assert len(hits)==129,hits
print('V11_CONTROL_HELPER_HITS',len(hits))
end
