...
R=Path('/home/sam/Programming/sqgi');A=R/'devdocs/internals/jit-performance-iterations-2026-09-09';D=A/'precise-native-cache-v12';O=Path('/tmp/sqgi-profile-20260909/source')
...
with tempfile.TemporaryDirectory(prefix='sqgi-v12-final-reconstruction-') as td:
 t=Path(td)
 for p in paths:
...
shutil.copy2(__file__,D/'sqgi-precise-v12-verify.py')
files={R/p for p in old};files.update(p for p in D.rglob('*') if p.is_file());files.update(p for p in (R/'build-jit-audit-20260909/precise-native-cache-v12').rglob('*') if p.is_file());files.update(R/p for p in changed)
