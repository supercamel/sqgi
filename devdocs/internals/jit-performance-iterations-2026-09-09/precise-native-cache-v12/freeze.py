...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'precise-native-release-v11'; D=A/'precise-native-cache-v12'; B=R/'build-jit-audit-20260909/precise-native-cache-v12'; S=Path('/tmp/sqgi-precise-native-cache-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-precise-v12-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-precise-v12-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-precise-v11-x64-tests.log'); b=fails(Path('/tmp/sqgi-precise-v12-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
...
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v12')/p)==previous[p]
...
manifest={p:h(R/p) for p in previous};manifest['test/cpp/test_sqjit_leaf.cpp']=h(R/'test/cpp/test_sqjit_leaf.cpp');assert len(manifest)==94
shutil.copy2(R/'test/cpp/test_sqjit_leaf.cpp',D/'source/test/cpp/test_sqjit_leaf.cpp')
(D/'source-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');shutil.copy2(P/'unchanged-benchmarks-and-scripts.json',D)
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v12')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'native-cache-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v12-reconstruct-') as td:
 t=Path(td)
...
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-precise-v12-{mode}-holdouts'),D/f'{mode}-holdouts')
