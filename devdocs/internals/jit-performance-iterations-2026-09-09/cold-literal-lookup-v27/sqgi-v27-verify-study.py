...
# Preserve the original flags. A fixed, predeclared replication can resolve
# only this specific records_4096 finding; every other regression stays open.
unresolved=dict(regressions);resolution=None
if regressions:
 investigation=S/'records-replication/analysis.json'
 if investigation.exists():
  evidence=json.loads(investigation.read_text())
  assert evidence['verified'] and evidence['all_blocks_retained'] and evidence['original_flagged_controls_retained'] and evidence['full56_unchanged']
  assert evidence['policy']==json.loads((S/'records-replication/policy.json').read_text())
  assert evidence['policy']['blocks']==4 and evidence['policy']['runs_per_block']==5 and evidence['policy']['no_optional_stopping']
  for name,want in evidence['result_hashes'].items():assert h(S/'records-replication'/name)==want,name
  assert len(evidence['result_hashes'])==8
  if evidence['all_eight_cases_material_regression_excluded']:
   for group,fields in list(unresolved.items()):
    if group not in ['application-w4','application-w5'] or any(names!=['records_4096'] for names in fields.values()):continue
    w=group[-1];row=evidence['warmups'][w]['records_4096']
   ...
   resolution={'report':'records-replication/analysis.json','sha256':h(investigation),'original_flags_retained':True,'criterion':'new-only and original-plus-new 95% upper bounds <=1.05 under the predeclared fixed replication','records_4096':{w:rows['records_4096'] for w,rows in evidence['warmups'].items()}}
