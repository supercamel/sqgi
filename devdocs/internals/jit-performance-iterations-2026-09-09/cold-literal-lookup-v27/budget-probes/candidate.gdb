...
out=pathlib.Path('/tmp/sqgi-v27-budget-probes/candidate');captured={};entries=collections.Counter();breakpoints=[]
...
run /tmp/sqgi-v27-budget-probes/functions.nut
python
assert len(captured)==40 and set(entries)==set(captured),(len(captured),len(entries))
