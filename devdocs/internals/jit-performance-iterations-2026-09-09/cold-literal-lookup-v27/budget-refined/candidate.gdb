...
out=pathlib.Path('/tmp/sqgi-v27-budget-refined/candidate');captured={};entries=collections.Counter();breakpoints=[]
...
run /tmp/sqgi-v27-budget-refined/functions.nut
python
assert len(captured)==16 and set(entries)==set(captured),(len(captured),len(entries))
