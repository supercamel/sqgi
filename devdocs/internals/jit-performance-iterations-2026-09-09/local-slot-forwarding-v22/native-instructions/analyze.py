...
D=Path('/tmp/sqgi-v22-native-profile');S=Path('/tmp/sqgi-local-slot-forwarding-study')
def trim(b):
...
for (file,offset),count in hits.most_common():
 ...
 category='register_move' if mnemonic=='fmov' else 'load' if mnemonic.startswith(('ldr','ldp','ldur')) else 'store' if mnemonic.startswith(('str','stp','stur')) else 'branch' if mnemonic.startswith(('b.','cb','tb')) or mnemonic in ['b','br','blr','ret'] else 'floating' if mnemonic.startswith('f') or mnemonic=='scvtf' else 'other'
