...
R=Path('/home/sam/Programming/sqgi'); A=R/'devdocs/internals/jit-performance-iterations-2026-09-09'; P=A/'receiver-entry-guards-v21'; D=A/'local-slot-forwarding-v22'; B=R/'build-jit-audit-20260909/local-slot-forwarding-v22'; S=Path('/tmp/sqgi-local-slot-forwarding-study'); O=Path('/tmp/sqgi-profile-20260909/source')
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  assert '100% tests passed, 0 tests failed out of 100' in Path(f'/tmp/sqgi-precise-v22-{mode}-{enabled}-tests.log').read_text()
 binary=Path('/tmp/sqgi-scalar-leaf-20260909/sqgi' if mode=='release' else '/tmp/sqgi-leaf-asan-strict-20260909.M32jMl/sqgi')
 d=json.loads(Path(f'/tmp/sqgi-precise-v22-{mode}-holdouts/results.json').read_text()); assert d['sha256']==h(binary)
...
a=fails(P/'sqgi-precise-v21-x64-tests.log'); b=fails(Path('/tmp/sqgi-precise-v22-x64-tests.log'));assert a==b and sum(a.values())==32,(a,b)
previous=json.loads((P/'source-manifest.json').read_text()); extra='test/cpp/test_sqjit_aarch64_emit.cpp'; assert h(O/extra)==h(Path('/tmp/sqgi-precise-before-v22')/extra); previous[extra]=h(O/extra); changed=[p for p,s in previous.items() if h(R/p)!=s]; assert len(changed)==4,changed
for p in changed:assert h(Path('/tmp/sqgi-precise-before-v22')/p)==previous[p]
for mode in ['release','asan']:
 for enabled in ['enabled','default']:
  detail=Path(f'/tmp/sqgi-precise-v22-{mode}-{enabled}-ctest-detail.log').read_text()
...
patch=''.join(''.join(difflib.unified_diff((Path('/tmp/sqgi-precise-before-v22')/p).read_text().splitlines(True),(R/p).read_text().splitlines(True),'a/'+p,'b/'+p)) for p in changed)
(D/'local-slot-delta.patch').write_text(patch)
...
with tempfile.TemporaryDirectory(prefix='sqgi-v22-reconstruct-') as td:
 t=Path(td)
...
for p in Path('/tmp').glob('sqgi-precise-v22-*.log'):shutil.copy2(p,D/p.name)
for mode in ['release','asan']:shutil.copytree(Path(f'/tmp/sqgi-precise-v22-{mode}-holdouts'),D/f'{mode}-holdouts')
