...
for w in [4,5]:
 ...
 for k,m in d['medians'].items():
  rows[k]=dict(medians_ns={label:v*1000 for label,v in m.items()},v22_over_audit=m['v22']/m['audit'],v22_over_v21=m['v22']/m['v21'],repeat_over_audit=m['v22_repeat']/m['audit'],repeat_over_candidate=m['v22_repeat']/m['v22'],ranges_ns={label:[min(v)*1000,max(v)*1000] for label,v in d['samples'][k].items()})
 result[f'w{w}']=rows
