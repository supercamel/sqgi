...
for w in [4,5]:
 for group in ['kernel','floats']:
  ...
  for k,m in d['medians'].items():
   per[k]={'medians_ns':{name:value*1000 for name,value in m.items()},'v22_over_audit':m['v22']/m['audit'],'v22_over_v21':m['v22']/m['v21'],'identical_repeat_over_v22':m['v22_repeat']/m['v22']}
  result[f'{group}-w{w}']={'per_case':per,'regressions_over_5pct_against_v21':[k for k,v in per.items() if v['v22_over_v21']>1.05],'geomean_v22_over_v21':statistics.geometric_mean(v['v22_over_v21'] for v in per.values())}
