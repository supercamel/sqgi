...
P=R/'devdocs/internals/jit-performance-iterations-2026-09-09/precise-native-copy-v14/profiles'
D=Path('/tmp/sqgi-precise-shared-exit-study/profiles');result={}
for name in ['recycle','transfer','publish','table_apply','apply']:
 ...
 assert sum(w==0xb9000289 for w in old)==len(expected),(name,'old tag publication count')
 result[name]={'guard_count':len(expected),'exact_resume_ips':found,'common_publications_before':len(expected),'common_publications_after':1,'mapped_bytes_before':len(oldraw),'mapped_bytes_after':len(raw),'decoded_code_bytes_before':oldend*4,'decoded_code_bytes_after':end*4,'bytes_saved':(oldend-end)*4,'expected_savings_same_hot_body':36*len(expected)-40}
 assert (oldend-end)*4==36*len(expected)-40,(name,result[name])
(D/'exit-code-verification.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
print('PASS: all guard branches select their exact bytecode IP and converge on one SIDE_EXIT publication/restoration; size reduction equals removal of duplicate cold returns.')
