...
S=Path('/tmp/sqgi-shared-helper-study');R=Path('/home/sam/Programming/sqgi');out={}
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
candidate_hash=h(R/'build-jit-audit-20260909/shared-helper-v28/sqgi')
for w in [4,5]:
 for group,count in [('kernel',15),('floats',7),('application',8),('application-seed17',8),('owners',10),('members',6),('methods',6)]:
  ...
  labels={'v27','v28','v28_repeat'}| (set() if pilot else {'audit'})
  ...
  for k,all_m in d['medians'].items():
   ...
   row={'medians_ns':{n:v*1000 for n,v in m.items()},'ranges_ns':{n:[min(v)*1000,max(v)*1000] for n,v in samples.items()},'v28_over_v27':m['v28']/m['v27'],'repeat_over_v27':m['v28_repeat']/m['v27'],'repeat_over_v28':m['v28_repeat']/m['v28']}
   if 'audit' in m:row['v28_over_audit']=m['v28']/m['audit']
   rows[k]=row
  out[f'{group}-w{w}']={'per_case':rows,'geomean_v28_over_v27':statistics.geometric_mean(v['v28_over_v27'] for v in rows.values()),'regressions_over_5pct_against_v27':[k for k,v in rows.items() if v['v28_over_v27']>1.05],'repeat_regressions_over_5pct_against_v27':[k for k,v in rows.items() if v['repeat_over_v27']>1.05]}
