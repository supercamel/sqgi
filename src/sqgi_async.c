...
void sqgi_async_register(HSQUIRRELVM v)
{
    ...
    static const char k_bootstrap[] =
        ...
        "sqgi.__make_async_wrapper <- function(bn, no_cancellable = false) {\n"
        ...
        "        local call_args = [self]\n"
        "        foreach (a in args) call_args.append(a)\n"
        "        if (!no_cancellable) call_args.append(c)\n"
        "        call_args.append(cb)\n"
        "        if (no_cancellable) {\n"
        ...
        "        try {\n"
        "          self[raw].acall(call_args)\n"
        "        } catch (with_cancellable_error) {\n"
        "          local no_cancellable_args = [self]\n"
        "          foreach (a in args) no_cancellable_args.append(a)\n"
        "          no_cancellable_args.append(cb)\n"
        "          self[raw].acall(no_cancellable_args)\n"
        "        }\n"
        "      },\n"
