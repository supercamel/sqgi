...
| `gtk4/`        | GTK 4         | `hello_world.nut`, `counter.nut`, `widgets.nut` (signal-exercising gallery), `bouncing_ball.nut` (Cairo-rendered animation via `Gtk.DrawingArea.set_draw_func`), `textbuffer_iters.nut` (headless `Gtk.TextBuffer`/`Gtk.TextIter` smoke demo) |
